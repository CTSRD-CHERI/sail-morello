...
\<comment> \<open>\<open>val chooseS : forall 'regs 'a 'e. SetType 'a => set 'a -> monadS 'regs 'a 'e\<close>\<close>
definition chooseS  :: \<open> 'a set \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)result*'regs sequential_state)set \<close>  where 
     \<open> chooseS xs s = ( Set.image ((\<lambda> x .  (Value x, s))) xs )\<close> 
  for  xs  :: " 'a set " 
  and  s  :: " 'regs sequential_state "


\<comment> \<open>\<open>val choose_boolS : forall 'regs 'e. unit -> monadS 'regs bool 'e\<close>\<close>
...
\<comment> \<open>\<open>val headS : forall 'rv 'a 'e. list 'a -> monadS 'rv 'a 'e\<close>\<close>
definition headS  :: \<open> 'a list \<Rightarrow> 'rv sequential_state \<Rightarrow>(('a,'e)result*'rv sequential_state)set \<close>  where 
     \<open> headS = ( (\<lambda>x .  
  (case  x of   x # _ => returnS x | [] => failS (''headM'') )))\<close>


\<comment> \<open>\<open>val tailS : forall 'rv 'a 'e. list 'a -> monadS 'rv (list 'a) 'e\<close>\<close>
definition tailS  :: \<open> 'a list \<Rightarrow> 'rv sequential_state \<Rightarrow>((('a list),'e)result*'rv sequential_state)set \<close>  where 
     \<open> tailS = ( (\<lambda>x .  
  (case  x of   _ # xs => returnS xs | [] => failS (''tailM'') )))\<close>
