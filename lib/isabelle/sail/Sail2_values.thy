...
\<comment> \<open>\<open>val prerr_bits : forall 'a. Bitvector 'a => string -> 'a -> unit\<close>\<close>
definition prerr_bits  :: \<open> 'a Bitvector_class \<Rightarrow> string \<Rightarrow> 'a \<Rightarrow> unit \<close>  where 
     \<open> prerr_bits dict_Sail2_values_Bitvector_a str v = ( prerr_endline (str @ string_of_bv 
  dict_Sail2_values_Bitvector_a v))\<close> 
  for  dict_Sail2_values_Bitvector_a  :: " 'a Bitvector_class " 
  and  str  :: " string " 
  and  v  :: " 'a "
...
record 'rv Register_Value_class=

  bool_of_regval_method ::" 'rv \<Rightarrow>  bool option " 

  regval_of_bool_method ::" bool \<Rightarrow> 'rv " 

  int_of_regval_method ::" 'rv \<Rightarrow>  int option " 

  regval_of_int_method ::" int \<Rightarrow> 'rv " 

  real_of_regval_method ::" 'rv \<Rightarrow>  real option " 

  regval_of_real_method ::" real \<Rightarrow> 'rv " 

  string_of_regval_method ::" 'rv \<Rightarrow>  string option " 

  regval_of_string_method ::" string \<Rightarrow> 'rv "
...
\<comment> \<open>\<open> Register operations which do not depend on polymorphic type \<close>\<close>
type_synonym( 'regstate, 'regval) register_ops ="
  (('regval \<Rightarrow> bool) * ('regstate \<Rightarrow> 'regval) *
   ('regval \<Rightarrow> 'regstate \<Rightarrow>  'regstate option))"

\<comment> \<open>\<open>val register_ops_of : forall 'st 'regval 'a.
    register_ref 'st 'regval 'a -> register_ops 'st 'regval\<close>\<close>
definition register_ops_of  :: \<open>('st,'regval,'a)register_ref \<Rightarrow>('regval \<Rightarrow> bool)*('st \<Rightarrow> 'regval)*('regval \<Rightarrow> 'st \<Rightarrow> 'st option)\<close>  where 
     \<open> register_ops_of reg = (
  (((\<lambda> x .  \<not> (((of_regval   reg) x) = None))),
   ((\<lambda> x . (regval_of   reg) ((read_from   reg) x))),
   ((\<lambda> x st .  map_option ((\<lambda> v . (write_to   reg) v st)) ((of_regval   reg) x)))))\<close> 
  for  reg  :: "('st,'regval,'a)register_ref "
...
\<comment> \<open>\<open>val mk_accessors : forall 'st 'v.
    (string -> maybe (register_ops 'st 'v)) -> register_accessors 'st 'v\<close>\<close>
definition mk_accessors  :: \<open>(string \<Rightarrow>(('v \<Rightarrow> bool)*('st \<Rightarrow> 'v)*('v \<Rightarrow> 'st \<Rightarrow> 'st option))option)\<Rightarrow>(string \<Rightarrow> 'st \<Rightarrow> 'v option)*(string \<Rightarrow> 'v \<Rightarrow> 'st \<Rightarrow> 'st option)\<close>  where 
     \<open> mk_accessors regs = (
  (((\<lambda> nm st .  map_option ( (\<lambda>x .  
  (case  x of (_, acc1, _) => acc1 st ))) (regs nm))),
   ((\<lambda> nm v st .  Option.bind (regs nm) ( (\<lambda>x .  
  (case  x of (_, _, put) => put v st )))))))\<close> 
  for  regs  :: " string \<Rightarrow>(('v \<Rightarrow> bool)*('st \<Rightarrow> 'v)*('v \<Rightarrow> 'st \<Rightarrow> 'st option))option "
